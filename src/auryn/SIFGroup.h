...
private:
	...
	AurynStateVector * bg_current_dendrite;
	auryn_vector_ushort * ref;
	unsigned short refractory_time;
	AurynFloat e_rest,e_rev_gaba,thr,tau_mem, tau_dendrite,r_mem,c_mem;
	AurynFloat tau_ampa,tau_gaba;
	AurynFloat scale_ampa, scale_gaba, scale_mem, scale_cursyn, scale_dendrite;

  AurynStateVector * dendrite;
  AurynStateVector * t_dendrite;
  AurynStateVector * t_mem;
...
public:
  ...
  NodeDistributionMode mode;
	...
	/*! \brief Controls the constant current input to the dendrite compartment (per default set so zero) to neuron i */
	void set_bg_current_dendrite(NeuronID i, AurynFloat current);

	/*! \brief Controls the constant current input  to the dendrite compartment to all neurons */
	void set_bg_currents_dendrite(AurynFloat current);
